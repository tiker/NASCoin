(function($, NRS, undefined) {
    	...
    	.done(function(response) {
    		if (response.timestamp) {
    			...
    			NRS.sendRequest("getTime", function(response) {
    				var serverTimestamp = Math.round(new Date(Date.UTC(2014, 2, 25, 12, 0, 0, 0) + response.time * 1000).getTime() / 1000);
	...
	NRS.downloadClientUpdate = function(version) {
	    ...
	    $("#nrs_update_iframe").attr("src", "http://download.nascoin.org/nas-client-" + NRS.downloadedVersion.versionNr + ".zip");
    ...
    NRS.calculateBlockchainDownloadTime = function(callback) {
    	...
    	try {
    		if (NRS.isTestNet) {
	    		var url = "http://test.nascoin.org:6876/nxt?requestType=getState";
    		} else {
    			var url = "http://www.nascoin.org:7876/nxt?requestType=getState";
    		}
	...
	$("#account_balance_modal").on("show.bs.modal", function(e) {
		if (NRS.accountBalance.errorCode) {
			...
			$("#account_balance_balance").html(NRS.formatAmount(NRS.accountBalance.balance/100) + " NAS");
			$("#account_balance_effective_balance").html(NRS.formatAmount(NRS.accountBalance.effectiveBalance/100) + " NAS");
			...
			if (!NRS.accountBalance.publicKey) {
				...
				$("#account_balance_warning").html("Your account does not have a public key! This means it's not as protected as other accounts. You must make an outgoing transaction to fix this issue. (<a href='#' data-toggle='modal' data-target='#send_message_modal'>send a message</a>, <a href='#' data-toggle='modal' data-target='#register_alias_modal'>buy an alias</a>, <a href='#' data-toggle='modal' data-target='#send_money_modal'>send Nas</a>, ...)").show();
			}
		}
	});
    ...
    NRS.processAccountModalData = function(account) {
    	...
    	if (balance == 0) {
    		...
    		$("#user_info_modal_balance").html(NRS.formatAmount(balance) + " NAS");
    	}	
    	...
    	if (currentTime - NRS.newsRefresh > 60*60*10) { //10 minutes before refreshing..
    		...
    		var settingsReddit = {
	    		...
	    		"filter": function(entry, tokens) {
	    			return tokens.title.indexOf("Donations toward") == -1 && tokens.title.indexOf("NAS tipping bot has arrived") == -1
	    		}
	    	};
	...
	$("#asset_order_modal").on('show.bs.modal', function (e) {
		...
		if (orderType == "Buy") {
			...
			var description = "Buy <strong>" + NRS.formatAmount(quantity, false, true) + " " + $("#asset_name").html() + "</strong> assets at <strong>" + NRS.formatAmount(price, false, true) + " NAS</strong> each.";
			var tooltipTitle = "As each asset is bought you will pay " + NRS.formatAmount(price, false, true) + " NAS, making a total of " + NRS.formatAmount(price*quantity, true, true) + " NAS once all assets have been bought.";
		} else {
			...
			var description = "Sell <strong>" + NRS.formatAmount(quantity, false, true) + " " + $("#asset_name").html() + "</strong> assets at <strong>" + NRS.formatAmount(price, false, true) + " NAS</strong> each.";
			var tooltipTitle = "As each asset is sold you will receive " + NRS.formatAmount(price, false, true) + " NAS, making a total of " + NRS.formatAmount(price*quantity, true, true) + " NAS once all assets have been sold.";
		}
		...
		$("#asset_order_total").html(NRS.formatAmount(price*quantity, true) + " NAS");
		$("#asset_order_fee_paid").html(NRS.formatAmount(fee) + " NAS");
	...
	NRS.forms.createPollComplete = function(response, data) {	
		...
		if (NRS.currentPage == "polls") {
			...
			var date = new Date(Date.UTC(2014, 3, 25, 12, 0, 0, 0)).getTime();
	...
	*/
    ...
    NRS.forms.sendMoneyComplete = function(response, data) {  
    	if (!(data["_extra"] && data["_extra"].convertedAccount) && !(data.recipient in NRS.contacts)) {
	    	$.growl("NAS has been sent! <a href='#' data-account='" + String(data.recipient).escapeHTML() + "' data-toggle='modal' data-target='#add_contact_modal' style='text-decoration:underline'>Add recipient to contacts?</a>", {"type": "success"});
    	} else {
    		$.growl("NAS has been sent!", {"type": "success"});
    	}
    }
    ...
    $("#register_alias_modal").on("show.bs.modal", function(e) {
    	...
    	if (alias) {
    		NRS.sendRequest ("getAliasURI", {"alias": alias}, function(response) {
	    		...
	    		} else if (/acct:(\d+)@nas/.test(response.uri) || /nacc:(\d+)/.test(response.uri)) {
    ...
    NRS.forms.assignAlias = function($modal) {
    	...
    	if (data.type == "account") {
    		if (!(/acct:(\d+)@nas/.test(data.uri)) && !(/nacc:(\d+)/.test(data.uri))) {
    			if (/^\d+$/.test(data.uri)) {
	    			data.uri = "acct:" + data.uri + "@nas";
    			} else {
    ...
    NRS.forms.setAliasType = function(type, uri) {
		...
		if (type == "uri") {
			...
			if (uri) {
				if (!(/acct:(\d+)@nas/.test(uri)) && !(/nacc:(\d+)/.test(uri))) {
					if (/^\d+$/.test(uri)) {
						$("#register_alias_uri").val("acct:" + uri + "@nas");
					} else {
	    				$("#register_alias_uri").val("");
					}
				} else {
			...
			$("#register_alias_help").html("The alias will reference the account number entered and can be used to send Nas to, messages, etc..").show();
		} else {
    ...
    NRS.createInfoTable = function(data, fixed) {
    	...
    	for (var key in data) {
    		...
    		if (key == "Quantity") {
    			...
    			value = NRS.formatAmount(value/100, true) + " NAS";
    		} else {
		...
		NRS.database.select("contacts", null, function(error, contacts) {			
			if (contacts.length) {
				...
				$.each(contacts, function(index, contact) {  
					...
					rows += "<tr><td><a href='#' data-toggle='modal' data-target='#update_contact_modal' data-contact='" + String(contact.id).escapeHTML() + "'>" + contact.name.escapeHTML() + "</a></td><td><a href='#' data-user='" + String(contact.accountId).escapeHTML() + "' class='user_info'>" + String(contact.accountId).escapeHTML() + "</a></td><td>" + (contact.email ? contact.email.escapeHTML() : "-") + "</td><td>" + contactDescription.escapeHTML() + "</td><td style='white-space:nowrap'><a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_money_modal' data-contact='" + String(contact.name).escapeHTML() + "'>Send Nas</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#send_message_modal' data-contact='" + String(contact.name).escapeHTML() + "'>Message</a> <a class='btn btn-xs btn-default' href='#' data-toggle='modal' data-target='#delete_contact_modal' data-contact='" + String(contact.id).escapeHTML() + "'>Delete</a></td></tr>";
				});
    ...
    NRS.getAccountError = function(accountId, callback) {    	
		NRS.sendRequest("getAccount", {"account": accountId}, function(response) {	
			if (response.publicKey) {
				callback({"type": "info", "message": "The recipient account has a public key and a balance of " + NRS.formatAmount(response.balance/100, false, true) + "NAS."});
			} else {
				if (response.errorCode) {
					...
					callback({"type": "warning", "message": "The recipient account does not have a public key, meaning it has never had an outgoing transaction. The account has a balance of " + NRS.formatAmount(response.balance/100, false, true) + " NAS. Please double check your recipient address before submitting."});
				}
			}
		});
    }    
    ...
    NRS.checkRecipient = function(account, modal) {
		...
		if (/^(NAS\-)?[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+\-[A-Z0-9]+/i.test(account)) {
			account = account.replace(/^NAS\-/i, "");
    ...
    NRS.getAccountBalance = function(firstRun) {
    	NRS.sendRequest("getAccount", {"account": NRS.account}, function(response) {    
    		...
    		if (response.errorCode) {
    			...
    			if (!NRS.accountBalance.publicKey) {
					$("#dashboard_message").addClass("alert-danger").removeClass("alert-success").html("<b>Warning!</b>: Your account does not have a public key! This means it's not as protected as other accounts. You must make an outgoing transaction to fix this issue. (<a href='#' data-toggle='modal' data-target='#send_message_modal'>send a message</a>, <a href='#' data-toggle='modal' data-target='#register_alias_modal'>buy an alias</a>, <a href='#' data-toggle='modal' data-target='#send_money_modal'>send Nas</a>, ...)").show();
    			} else {
	    			$("#dashboard_message").hide();
    			}
    ...
    NRS.generateLocalToken = function(website, secretPhrase) {
		...
		var timestamp 		= parseInt(((new Date()).getTime() - new Date(Date.UTC(2014, 3, 25, 12, 0, 0, 0)) + 5000) /1000, 10);
    ...
    NRS.formatTimestamp = function(timestamp, date_only) {
        var date = new Date(Date.UTC(2014, 2, 25, 12, 0, 0, 0) + timestamp * 1000);
